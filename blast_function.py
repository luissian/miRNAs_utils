...
class BlastFunction(Function):
    ...
    def apply(self, elem):  #  elem = tupla que nos devuelve el reader
        def collect_data(out_lines, forward_direction):
            matches_found = []
            for out_line in out_lines:
                line_split = out_line.split('\t')
                try:
                    if forward_direction:
                        if int(line_split[9]) < int(line_split[10]):
        ...
        result = {specie_name:{'compl':'','direct':''}}
        ...
        if len (out_lines) > 0 :
            result[specie_name]['compl'] = collect_data (out_lines, False)
            '''
            matches_found = []
            for out_line in out_lines:
                line_split = out_line.split('\t')
        ...
        cline = NcbiblastnCommandline(db=blast_db_name, task='blastn-short', strand='plus', evalue=0.001,  outfmt =  self.__params ,num_threads=1, query=query_fasta_file)

        out, err = cline()
        out_lines = out.splitlines()
        if len (out_lines) > 0 :
            result[specie_name]['direct'] = collect_data(out_lines, True)
            #result[specie_name]['direct'] = collect_data(out_lines)
